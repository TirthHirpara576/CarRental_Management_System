...
const ManageCars = () => {
    ...
    const [cars, setCars] = useState([]); // for storing all cars data 
    const fetchOwnerCars = async () => {
        try {
            const { data } = await axios.get("/api/owner/cars"); // Call API :- get all cars of the owner from the backend server
            if(data.success) {
                setCars(data.cars);
            }
            else {
    ...
    const toggleCarAvailability = async (carId) => {
        try {
            ...
            if(data.success) {
                ...
                fetchOwnerCars(); // Refresh the car list after toggling availability
            }
            else {
                toast.error(data.message);
            }
    ...
    }, [isOwner]); // execute this function whenevr component is loaded
